...
def sequence(m, j, adjacency_matrix, pheromone_matrix, machine_matrix, availability_matrix, availability_time_matrix, current_time, num_day, device="cpu"):
    ...
    machine_times = torch.zeros(num_machines, device=device, dtype=torch.float) # when start time is 0
    #machine_times = torch.full((num_machines,), current_time, dtype=torch.float, device=device) # when start time is not 0 Scenerio 1
